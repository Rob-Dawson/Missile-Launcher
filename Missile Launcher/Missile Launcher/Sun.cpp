...
Sun::Sun()
	...
	m_pPixelShader{ nullptr }, m_pVertexShader{ nullptr }, m_pInputLayout{ nullptr }, lightPos{ 0.0f,0.0f,0.0f }

{
}

bool Sun::createMeshSun(const Renderer &renderer, const D3D11_FILL_MODE *const wireFrame)
{
	bool hr{ 0 };
	...
	vertexDesc.ByteWidth = sizeof(SunVertex) * ARRAYSIZE(vertices);
	...
	constantBufferDesc.ByteWidth = sizeof(ConstantBufferSun);
	...
	// Set vertex buffer
	const UINT stride = sizeof(SunVertex);
	const UINT offset = 0;
...
bool Sun::createShadersSun(const Renderer & const renderer)
{
	...
	Shape::createShaders(renderer, layout, layoutSize, L"Sun.fx");
...
void Sun::draw(ID3D11DeviceContext *const deviceContext, const DirectX::XMMATRIX *const viewMatrix, const DirectX::XMMATRIX *const projectionMatrix, const float time)
{

	lightPos = XMVectorSet(20.0f, 50.0f, -10.0f, 0.0f);
	m_WorldMatrix = DirectX::XMMatrixIdentity();
	
	const XMMATRIX mRotate = XMMatrixRotationZ(time * 1.5f);
	lightPos = XMVector3Transform(lightPos, mRotate);

 	m_WorldMatrix *= XMMatrixTranslation(20.0f,5.0f, 10.0f);
	m_WorldMatrix *= mRotate;

	ConstantBufferSun cb{};
...
Sun::~Sun()
{
	...
	{
		MessageBox(nullptr, "Error in Sun while shutting down", "Error in Sun", MB_OK);
	}
}
